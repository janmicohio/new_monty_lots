...
class S3Sync {
  constructor(config) {
    ...
    console.log(`✓ S3 sync configured for bucket: ${this.bucket}${this.endpoint ? ` (endpoint: ${this.endpoint})` : ''}`);
  }
  ...
  async sync() {
    ...
    try {
      console.log(`🔄 Starting S3 sync from bucket: ${this.bucket}${this.prefix ? `/${this.prefix}` : ''}`);
